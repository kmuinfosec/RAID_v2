...
def SummaryGraph(result_path):
    ...
    for i in group_key_df["group"].tolist():
        ...
        if len(temp_df[temp_df["cluster"] == -1]) == 0:
            ...
            c_packets.append(
                temp_df[temp_df["cluster"] == -1]["cluster_packet"].values[0]
            )
        if len(temp_df[temp_df["cluster"] != -1]) != 0:
            c_packets.append(
                sorted(
                    temp_df[temp_df["cluster"] != -1]["cluster_packet"].tolist(),
                    reverse=True,
                )[0]
            )
    ...
    plt.yscale("log")
    plt.xticks(
        np.arange(bar_width / 2, len(x) + bar_width / 2, 1),
        x,
        fontsize=8,
        rotation=25,
        ha="right",
